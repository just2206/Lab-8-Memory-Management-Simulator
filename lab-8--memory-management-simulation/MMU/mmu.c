...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    // 1. Find a suitable block in FREE_LIST
    
    // For FIFO (First Fit), the list is unsorted, so we search for the first fit.
    // For BESTFIT and WORSTFIT, the freelist is sorted by size, but we still use
    // list_get_index_of_by_Size to find the *first* block that fits.
    // list_get_index_of_by_Size finds the first block >= blocksize.
    index_to_remove = list_get_index_of_by_Size(freelist, blocksize);
    ...
    // 2. Remove the block from the FREE_LIST
    free_block = list_remove_at_index(freelist, index_to_remove);
    
    // The original block's end address before fragmentation
    int original_end = free_block->end; 

    // 3. set the blk.pid = pid
    free_block->pid = pid;
    
    // 4. set the blk.end = blk->start + blocksize - 1
    int fragment_start = free_block->start + blocksize;
    free_block->end = free_block->start + blocksize - 1;
    
    // 5. add the blk to the ALLOC_LIST in ascending order by address.
    list_add_ascending_by_address(alloclist, free_block);
    
    // 6. Deal with the remaining left over memory (fragment).
    // The fragment exists only if there is remaining space.
    if (free_block->end < original_end) {
        ...
        // c. set the fragment->start = the blk.end + 1 (or fragment_start)
        ...
        } else if (policy == 2) { // BESTFIT (Ascending by size)
            list_add_ascending_by_blocksize(freelist, fragment);
        } else if (policy == 3) { // WORSTFIT (Descending by size)
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    int index_to_remove = list_get_index_of_by_Pid(alloclist, pid);
    ...
    // 2. if so, remove it
    block_t *deallocated_block = list_remove_at_index(alloclist, index_to_remove);
    
    // 3. set the blk.pid back to 0
    deallocated_block->pid = 0;
    
    // 4. add the blk back to the FREE_LIST based on policy.
    if (policy == 1) { // FIFO (First Fit)
        list_add_to_back(freelist, deallocated_block);
    } else if (policy == 2) { // BESTFIT (Ascending by size)
        list_add_ascending_by_blocksize(freelist, deallocated_block);
    } else if (policy == 3) { // WORSTFIT (Descending by size)
        // list_add_descending_by_blocksize's implementation in list.c is provided,
        // but it looks slightly off (e.g., blocksize calculation is missing +1).
        // Assuming you correct it to use the +1 (like the one in the list.c file):
        // int newblk_size = blk->end - blk->start + 1;
        list_add_descending_by_blocksize(freelist, deallocated_block);
    }
}
