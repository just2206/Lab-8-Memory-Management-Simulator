...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  ...
  // Case 1: List is empty or new node goes at the front
  ...
  // Stop when current->next is NULL OR newblk->start is less than or equal to the next node's start
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  // Case 1: List is empty or new node goes at the front
  // Use compareSize to check if newblk_size is <= current head's blocksize
  ...
  // Traverse to find the correct insertion point
  // Stop when current->next is NULL OR newblk_size is less than or equal to the next node's size
  while (current->next != NULL) {
    if (compareSize(newblk_size, current->next->blk)) {
      break;
    }
    current = current->next;
  }
...
void list_coalese_nodes(list_t *l){ 
  ...
  while (current != NULL) {
    // Check if the two blocks are physically adjacent: prev.END + 1 == current.START
    if (prev->blk->end + 1 == current->blk->start) {
      // 1. Merge them by setting prev.END = current.END
      prev->blk->end = current->blk->end;
      
      // 2. Point prev.NEXT to current.NEXT (skipping over current)
      prev->next = current->next;
      
      // 3. Free current node
      node_free(current);
      
      // 4. Update current to the next node for the next iteration
      current = prev->next; 
      
      // NOTE: prev remains the same because it might be adjacent to the new 'current'
      // Example: A-B-C. Merge A and B to A'. Check A' and C.
    } else {
      // Not adjacent, advance both prev and current
      prev = current;
      current = current->next;
    }
  }
